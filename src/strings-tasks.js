...
// return value instanceof String || (typeof value === 'string' && value.constructor === String);
...
function removeLastOccurrences(str, value) {
  ...
  const afterLast = str.substring(lastIndex + value.length);
...
function startsWith(str, substr) {
  ...
  // const firstOccurrence = str.firstIndexOf(substr);
  // if (str.startsWith(firstOccurrence)) {
  //   return true;
  // }
...
function orderAlphabetically(str) {
  ...
  const split = str.split('');
  const sorted = split.sort((a, b) => a.localeCompare(b)).join('');
  return sorted;
}
...
function isPalindrome(str) {
  const regExp = /[\W_]/g;
  const cleanedStr = str.toLowerCase().replace(regExp, '');
